module.exports = function (RED) {
    ...
    function WhatsappClient(config) {
        ...
        function connectWA() {
            ...
            sock.ev.on('connection.update', (update) => {
                ...
                if (connection === 'close') {
                    ...
                    if (lastDisconnect && lastDisconnect.error && lastDisconnect.error.output && (lastDisconnect.error.output.statusCode === 410 || lastDisconnect.error.output.statusCode === 428 || lastDisconnect.error.output.statusCode === 515)) {
                        client = connectWA();
                    } else {
                        if (lastDisconnect && lastDisconnect.error && lastDisconnect.error.output && (lastDisconnect.error.output.statusCode === 401)) {
                            unlinkSync(DIR + '/session.json');

                            setTimeout(() => {
                                const tmp = useSingleFileAuthState(DIR + '/session.json');
                                state = tmp.state;
        ...
        node.sendimage = function (to, message, url, nodeId = false) {
            ...
            client.sendMessage(to, { image: { url: url, caption: message } }).then((r) => {
                if (nodeId) {
